...
import { GalleryPatternCTrailingCell } from "#components2/playground/pattern-c-section-gallery/gallery-pattern-c-trailing-cell.client"
...
export function PlaygroundPatternCSectionGalleryContent() {
  return (
    <div className="flex flex-col gap-8 p-6">
      <header className="flex flex-col gap-2">
        <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">
          Playground · Pattern C
        </p>
        <h1 className="text-2xl font-semibold tracking-tight">
          Governed Pattern C section gallery
        </h1>
        <p className="max-w-2xl text-sm text-muted-foreground">
          Reference states for <code>GovernedPatternCListSection</code> (Wave
          C1–C3): forbidden, invalid config, empty, ready table, and
          trailing-action metadata with disabled tooltips.
        </p>
      ...
      <div className="flex flex-col gap-10">
        <section id="pattern-c-forbidden" className="flex flex-col gap-2">
          ...
          <p className="text-sm text-muted-foreground">
            <code>parentAccessAllowed=false</code> — section-level denial before
            parse.
          </p>
          <GovernedPatternCListSection
        ...
        <section
          id="pattern-c-trailing-disabled"
          className="flex flex-col gap-2"
        >
          <h2 className="text-lg font-semibold">Ready + trailing metadata</h2>
          <p className="text-sm text-muted-foreground">
            Row 1: <code>trailingAction.state=disabled</code> with tooltip. Row
            2: ready.
          </p>
          <GovernedPatternCListSection

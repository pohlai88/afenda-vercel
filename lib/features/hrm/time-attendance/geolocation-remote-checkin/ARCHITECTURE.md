...
Traceability for **HRM-GEO-001 … HRM-GEO-032** and acceptance criteria **1–30** lives in code, not only this document:

| Artifact | Role |
| -------- | ---- |
| `geolocation-spec-map.shared.ts` | Stable area slug per requirement code |
| `geolocation-acceptance-coverage.shared.ts` | `shipped` / `partial` / `deferred` + evidence paths |

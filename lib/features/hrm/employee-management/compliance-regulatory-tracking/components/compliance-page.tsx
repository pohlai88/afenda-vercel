...
> = {
  ...
  queued: "default",
  submitted: "outline",
  // `acknowledged` is the terminal compliance state ÔÇö use the brand-primary
 ...
 * "Acknowledged Mar 5 ┬À KWSP ┬À manual". Each segment falls back gracefully so
  ...
  return (
    <span className="inline-flex items-center gap-2 text-xs text-muted-foreground">
      ...
      {authorityPayloadHash && (
        ...
        // tooltip. NOT a click-to-copy yet ÔÇö that becomes a follow-up if HR
        // asks for it.
        <code
...
// ---------------------------------------------------------------------------
// Delivery diagnostics (Phase 3F) ÔÇö surfaces last attempt status, http code,
...
// ---------------------------------------------------------------------------
// Mark acknowledged form (Phase 3H ÔÇö closes the lifecycle once HR receives
  ...
  return (
    <div className="flex flex-wrap items-start gap-3 py-2 text-sm">
      ...
      {detailHref && (
        ...
        // stays at the navigation default (off ÔÇö see i18n/navigation.tsx).
        <Button asChild size="sm" variant="ghost">
          <Link
            href={detailHref}
...
export function CompliancePage({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Period selector ÔÇö lightweight native select to avoid heavy client bundle */}
      {allPeriods.length > 0 && (
        <Card>
          <CardHeader>
      ...
      {!period ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-medium">
              {t("evidenceRegister")} ÔÇö {period.periodStart} to{" "}
              {period.periodEnd}
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {/* Audit footer ÔÇö rule pack version info */}

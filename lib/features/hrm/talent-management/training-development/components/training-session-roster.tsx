          ...
          <div key={session.id} className="rounded-lg border p-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="font-medium">
                  {session.code} ÔÇö {session.title}
                </p>
                <p className="text-xs text-muted-foreground">
                  {session.courseName} ┬À {session.location}
                </p>
              </div>
              {open ? (
            ...
            <Table>
              ...
              <TableBody>
                <TableRow>
                  <TableCell colSpan={2}>
                    {formatDateTime(session.scheduledStartAt)} ÔÇö{" "}
                ...
                {roster.map((row) => (
                  <TableRow key={row.id}>
                    <TableCell colSpan={2}>
                      {row.employeeNumber} ÔÇö {row.employeeName}
                    </TableCell>
                    <TableCell>{row.attendance ?? "ÔÇö"}</TableCell>

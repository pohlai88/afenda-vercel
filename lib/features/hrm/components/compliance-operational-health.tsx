...
/**
 * Phase 3L — Cross-period operational health.
 *
 ...
 *   - Drill-downs reuse the Phase 3K per-evidence timeline route — no
...
export async function ComplianceOperationalHealth({
  ...
  if (snapshot.rowsConsidered === 0) {
    // Operationally distinct from "everything is calm" — render the
  ...
  return (
    <Card size="sm">
      ...
      <CardContent className="flex flex-col gap-6">
        {/*
         * COUNTER STRIP — every flow surfaced. Non-attention buckets
        ...
        {/*
         * ATTENTION SECTION — only renders if at least one attention
        ...
        {/*
         * AUDIT FOOTER — surfaces window clamp + computation time so HR
...
// ---------------------------------------------------------------------------
// Internal label bundle — resolved once at the parent and passed down so
...
type ComplianceHealthLabels = {
   ...
   * acknowledgement". Only attention buckets have this copy — the
  ...
  /** Phase 3O — severity tier short label for stuck sample badges. */
...
function SampleRow({
  ...
  // Severity tier is only meaningful for stuck rows — `failing` and

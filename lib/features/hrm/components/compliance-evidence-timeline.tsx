...
/**
 * Phase 3K — Per-evidence operational chronology.
 *
 ...
 *
 * No per-row formatting reaches inline tokens — geometry, color, and
...
/**
 * Visual variant selection — calm by default; failure paths get
...
function badgeVariantForKind(
  ...
  switch (kind) {
    ...
    case "aging_critical":
      // `aging_critical` is regulator-visible exposure — same visual
...
export async function ComplianceEvidenceTimeline({
  ...
  // with the closed kind enum — adding a new kind without an i18n entry
...
// ---------------------------------------------------------------------------
// Per-entry detail strip — pure rendering, no client interactivity yet.
// ---------------------------------------------------------------------------
...
type FacetLabels = {
  ...
  severityTier: string
  /** Phase 3T facets — operator-issued export download (chain of custody). */
  format: string
  responseHash: string
  /**
   * Phase 3U facets — provenance + lifecycle state DISCARDED on an
...
function EntryDetails({
  ...
  switch (entry.kind) {
    ...
    case "aging_critical": {
      // System-observed tier crossing — surface ONLY the facets HR
      ...
      // `bucketBefore` / `bucketAfter` — those are internal
    ...
    case "regenerated": {
      // Phase 3U — surface the operationally-meaningful prior state that
      // the in-place UPDATE just discarded. We deliberately render
      // PRIOR hashes (not new ones) and the PRIOR submission +
      // acknowledgement state — answering "the file Bob downloaded last
      ...
      // ISO 8601 string — regulator-readable, no localization (audit

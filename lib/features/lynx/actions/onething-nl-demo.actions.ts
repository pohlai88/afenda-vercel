...
import { buildLynxOneThingNlDemoGenerateSqlSystemPrompt } from "../data/onething-nl-demo-prompt.server"
import {
...
export async function generateLynxOneThingNlDemoSqlAction(
  question: string
  ...
  try {
    const { object } = await generateObject({
      ...
      system: buildLynxOneThingNlDemoGenerateSqlSystemPrompt(
        org.organizationId
      ),
    ...
    logUnexpectedServerError("lynx_onething_nl_demo_generate_sql_failed", err, {
      scope: "action.lynx.nl_sql_demo.onething",
      "erp.module": "lynx",
...
export async function executeLynxOneThingNlDemoSqlAction(
  sqlQuery: string
): Promise<
...
> {
  ...
  try {
    ...
    logUnexpectedServerError("lynx_onething_nl_demo_execute_sql_failed", err, {
      scope: "action.lynx.nl_sql_demo.onething",
      "erp.module": "lynx",
      organizationId: org.organizationId,
    })

...
export function OneThingNlDemoClient() {
  const t = useTranslations("Dashboard.Lynx.nlDemoOneThing")
  ...
  const runPipeline = useCallback(async () => {
    ...
    try {
      const gen = await generateLynxOneThingNlDemoSqlAction(q)
      if (!gen.ok) {
        setError(gen.error)
        return
      }
      ...
      const ran = await executeLynxOneThingNlDemoSqlAction(gen.sql)
      if (!ran.ok) {
        setError(ran.error)
        return
      }
  ...
  return (
    <section className="flex flex-col gap-4 rounded-2xl border bg-card p-4">
      ...
      <div className="grid gap-2">
        <label className="text-sm font-medium" htmlFor="lynx-onething-nl-q">
          {t("labelQuestion")}
        </label>
        <Input
          id="lynx-onething-nl-q"
          value={question}

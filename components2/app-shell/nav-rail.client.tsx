...
function SearchResultsList({
  ...
  recentResults: AppShellRailRecent[]
  onSelect: () => void
}) {
  const hasAny =
    navResults.length > 0 ||
...
type RailSearchProps = {
  ...
  navSections: AppShellRailNavSection[]
  recents?: AppShellRailRecent[]
  placeholder: string
  ariaLabel: string
...
function RailSearch({
  ...
  const recentResults = useMemo<AppShellRailRecent[]>(() => {
  ...
  return (
    <>
      ...
      {isActive ? (
        <div className="af-workbench-rail-scroll min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-2.5 py-0.5 [--radius:var(--radius-xl)]">
          <SearchResultsList
            navResults={navResults}
            memoryResults={memoryResults}
            recentResults={recentResults}
...
function NavItemWithLaneMenu({
  ...
  item: AppShellRailNavSection["items"][number]
  mode: "primary" | "sub"
  collapsed: boolean
}) {
  const addItem = useLaneMemoryStore((s) => s.addItem)

  // Collapsed mode: no visible label to capture, skip right-click menu.
  if (collapsed) {
    return mode === "primary" ? (
      <PrimaryNavItem item={item} />
    ) : (
      <AppShellNavItem item={item} collapsed={collapsed} />
    )
  }
...
function LocalLaneRow({
  ...
  const otherLanes = (["pinned", "urgent", "todo"] as const).filter(
    (l) => l !== item.lane
  )
  ...
  return (
    ...
    >
      <input
        ...
        placeholder={`Add to ${LANE_LABELS[lane]}…`}
        maxLength={160}
...
function RailViewsSection({
  ...
  views: ReadonlyArray<AppShellRailView>
  heading?: string
}) {
  const pathname = usePathname()
...
function RailRecentsSection({
  ...
  recents: ReadonlyArray<AppShellRailRecent>
  heading?: string
}) {
...
// ---------------------------------------------------------------------------
// Filter helper (mirrors filterWorkbenchRailNavSections)
// ---------------------------------------------------------------------------

function filterNav(
  sections: AppShellRailNavSection[],
  query: string
): AppShellRailNavSection[] {
  const q = query.trim().toLowerCase()
...
// ---------------------------------------------------------------------------
// AppShellRail (public export)
// ---------------------------------------------------------------------------

type AppShellRailProps = {
  config: AppShellRailConfig
  collapsed: boolean
  /**
   * "primary" — render nav items with SidebarMenuButton (requires SidebarProvider).
   *             Tooltip + collapse size are automatic. Use in AppShell main sidebar.
   * "sub"     — render nav items with context-free AppShellNavItem.
   *             Use in AppSubLayout in-flow aside (no SidebarProvider).
   */
  mode?: "primary" | "sub"
}
...
export function AppShellRail({
  ...
  const displayNav = useMemo(() => filterNav(nav, navQuery), [nav, navQuery])
  ...
  return (
    ...
    // same pattern as WorkbenchRail's <aside>. overflow-hidden is required

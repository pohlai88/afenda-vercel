...
5. [ERP / full-stack stack](#5-erp--full-stack-stack-this-repo) — [Neon + Vercel + pgvector (checklist)](#neon--vercel--pgvector-checklist) · [locale-first application surface](#locale-first-application-surface)
...
| **Other focused rules**       | `.cursor/rules/nextjs-best-practices.mdc`, `i18n-directory.mdc`, `lynx-directory.mdc`, `images.mdc`, `brand-assets.mdc`, `iam-directory.mdc`, `dev-directory.mdc` (`components/dev/**`), `testing-directory.mdc` (`tests/**`), `knip-directory.mdc` (`knip.json`, `package.json`, `scripts/**`), `registry-bases-parity.mdc`, `figma-code-connect-workflow.mdc` |
...
| `pnpm knip` / `pnpm knip:production` | Dead code & dependency graph ([Knip](https://knip.dev)) — **strict** (`knip.json`: unused files/deps, unresolved imports, export/type/duplicate issues as errors; `treatConfigHintsAsErrors`). Targeted **`ignoreIssues`** / **`ignoreDependencies`** only for intentional barrels, shadcn shelf, and fixtures. **`knip:production`** = ship-only |
...
- **Lynx (machine layer):** [`lib/features/lynx/`](lib/features/lynx/) — ERP **product module** for grounded **Truth Retrieval** (Phase 1) and future Operating Briefs, **Canonical Intake**, and Decision Operator. **`#features/knowledge`** remains the **substrate** (pgvector, `knowledge_chunk`, embeddings); Lynx **composes** it via **`#features/knowledge`** barrel imports only. Streaming HTTP lives under **`app/api/erp/lynx/*`**. Product lockup **Lynx** + **The Machine**; **Vercel AI SDK** is implementation-only (see **`.cursor/rules/lynx-directory.mdc`**). Stable audit strings and layer ids: **`lynx.contract.ts`** (`erp.lynx.*`).
...
6. **Smoke test:** sign in → **`/{locale}/o/{orgSlug}/dashboard/lynx`** (Truth Retrieval) and **`…/dashboard/knowledge`** (chunk substrate / redirect target) → add chunks → run truth search. **`vercel env run -e production -- next build`** can validate a prod-shaped build with pulled secrets locally ([docs](https://vercel.com/docs/cli/env)).
...
- **Server Actions (admin-gated):** [`actions/import-jobs.actions.ts`](lib/features/org-admin/actions/import-jobs.actions.ts) exposes **`createOrgImportJob`** (Tier B — parses CSV, stages rows + validation failures, never applies anything), **`runOrgImportJob`** (Tier A — applies all `pending` rows synchronously and transitions to `completed`), **`cancelOrgImportJob`** (Tier B). All three guard with **`requireOrgSession` + `canActInOrganization(..., "admin")`**, write IAM audit events on success, and revalidate via **`toLocaleOrgAdminRevalidatePattern("/integrations")`**. Audit action names: **`org.import.job.create | run | complete | cancel`**.
...
**Mental model:** the global **`/{locale}/admin/*`** surface is a registry-driven sibling of the organization workbench, not a marketing page with a single user list. It is the operator console for cross-tenant identity and platform telemetry. Tenant-scoped governance still lives under **`/o/{orgSlug}/admin`**.

- **URL surface:** **`/{locale}/admin/{section}`** under [`app/[locale]/admin/`](app/[locale]/admin/) (overview, users, organizations). Layout [`app/[locale]/admin/layout.tsx`](app/[locale]/admin/layout.tsx) enforces **`requireGlobalAdminSession()`** + **`requireRecentAuthStepUp`** before rendering.
- **Module:** [`lib/features/platform-admin/`](lib/features/platform-admin/) — public door **`#features/platform-admin`** ([`index.ts`](lib/features/platform-admin/index.ts)). Same shape as `org-admin` (capability filename prefixes, `actions` / `data` / `components` / `schemas` only); cross-tenant logic lives here, never in `app/[locale]/admin/*` route files.

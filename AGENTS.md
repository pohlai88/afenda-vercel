...
| Afenda Demo Showcase           | **Layer 1** `app/[locale]/demo/` · **Layer 2** `#features/demo` · **Layer 3** `#components2/demo` · `/{locale}/demo/*` public read-only fixtures · [ADR-0041](docs/decisions/0041-demo-showcase-three-lanes.md) · [docs/dev/demo-surfaces.md](docs/dev/demo-surfaces.md) · `pnpm dev:seed:demo-erp` for Lane B portal E2E |
...
| `pnpm dev:stack`                      | Multi-port local stack (fast default: local dotenv). **`pnpm dev:stack:full`** adds Vercel preflight + `vercel env run` — ADR-0039                                                                                                          |
...
Configure MCP servers in `~/.cursor/mcp.json` (global; no project `.cursor/mcp.json`). Servers include `Neon`, `vercel`, `next-devtools`, `context7`, `sentry`, `shadcn`, and others. Never put secrets in docs — use `${env:…}` in global config.

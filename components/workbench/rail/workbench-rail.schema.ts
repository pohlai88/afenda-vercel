 ...
 * surface in `workbench-rail.types.ts` is derived from it via `z.infer<>`,
 ...
 * `ReactNode` and cannot be expressed in Zod. See `workbench-rail.types.ts`
 * for the composed `WorkbenchRailSlots` type that adds `footer`.
 */

// ---------------------------------------------------------------------------
// Icon registry — the source of truth for serializable nav icon tokens.
// `workbench-rail-icon-map.ts` resolves each id to a Lucide component.
// ---------------------------------------------------------------------------
...
export const workbenchRailNavItemSchema = z.object({
 ...
 * `components/workbench/rail/` sits below `lib/features/` in the import

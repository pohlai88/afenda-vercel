...
export type WorkbenchUtilityWidgetRegistryEntry = NexusUtilityCatalogEntry & {
...
export const NEXUS_UTILITY_WIDGET_REGISTRY: Record<
  ...
  WorkbenchUtilityWidgetRegistryEntry
> = Object.fromEntries(
  NEXUS_UTILITY_CATALOG.flatMap((entry) =>
    entry.widget !== null && isInstalledNexusRightUtilityWidgetId(entry.id)
      ? [[entry.id, entry as WorkbenchUtilityWidgetRegistryEntry]]
      : []
  )
) as Record<NexusRightUtilityWidgetId, WorkbenchUtilityWidgetRegistryEntry>
...
export function getRightUtilityCatalogEntry(
  id: NexusUtilityWidgetId
): WorkbenchUtilityWidgetRegistryEntry | null {
...
export function migrateUtilityWidgetPrefs(
  ...
  if (!("right.integrations" in prefs))
    return prefs as UtilityWidgetVisibilityPrefs
...
export function getVisibleRightUtilityWidgetIds(input: {
  ...
  return (
    ...
    ) as WorkbenchUtilityWidgetRegistryEntry[]
  )
    .filter((entry) =>

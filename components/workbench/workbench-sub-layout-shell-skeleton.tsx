...
type WorkbenchSubLayoutShellSkeletonProps = {
  /** Visible status for assistive tech while the nested workbench shell streams. */
  statusLabel?: string
}

/**
 * Layout-shaped Suspense fallback for nested `AppSubLayout` routes — mirrors
 * [`WorkbenchSubLayoutClient`](./workbench-sub-layout.client.tsx) rail + main
 * column geometry so CLS stays low when Tier B data resolves.
 *
 * @see https://nextjs.org/docs/app/guides/streaming
 */
export function WorkbenchSubLayoutShellSkeleton({
  statusLabel = "Loading workbench",
}: WorkbenchSubLayoutShellSkeletonProps) {
  return (
    <div
    ...
    >
      ...
      <div
        className={`flex min-w-0 flex-1 flex-col bg-background ${WORKBENCH_CONTENT_PANE_INSET_CLASS}`}
      >

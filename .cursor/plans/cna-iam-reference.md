...
| Invitations & membership UX | Tenant invitations, roles | Better Auth **[Organization plugin](https://www.better-auth.com/docs/plugins/organization)** — already enabled in [`lib/auth/config.server.ts`](../../lib/auth/config.server.ts); UI **partial** ([`app/account/organization/page.tsx`](../../app/account/organization/page.tsx) read-only summary + member list) | **WP-05:** invite/update-role flows + `org.*` audit events still deferred. |

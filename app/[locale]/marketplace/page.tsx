...
/**
 * Marketplace overview — the single calm landing page that explains
 * the registry, summarizes what is currently visible to *this*
 * operator, and orients them toward the seven categories.
 *
 * Intentional shape:
 *
 *   - "Featured in your utility bar" reproduces the L1 rail in card
 *     form so the page itself answers "what am I seeing right now,
 *     and why?" without making the user click a category first.
 *
 *   - "Browse all utilities" lists everything else (capped to the
 *     full utilities catalog) with a clear toggle affordance —
 *     the v1 Marketplace surface manages personal preferences only.
 *
 *   - "Other categories" surfaces the architectural visibility for
 *     the remaining six categories so the registry contract is
 *     legible long before each module ships.
 *
 *   - "Adopted across Afenda" is gated by `MARKETPLACE_METRICS_K_ANONYMITY_THRESHOLD`
 *     and only renders capabilities that are above the threshold.
 */
export default async function MarketplaceOverviewPage(
  _props: PageProps<"/[locale]/marketplace">
) {
  ...
  // Chain-wide metrics — paged through the cached resolver so we hit
  // the DB at most once per capability per `MARKETPLACE_METRICS_REVALIDATE_SECONDS`.
  ...
  return (
    ...
    >
      ...
      <section className="flex flex-col gap-3">
        ...
        <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {CAPABILITY_CATEGORIES.filter(isCapabilityCategory).map((cat) => (
            <li key={cat}>
              <article className="flex h-full flex-col justify-between gap-3 rounded-2xl border border-border/60 bg-background/80 p-4">
      ...
      {viewer.isAdmin ? (
        <section className="rounded-2xl border border-dashed border-border/60 bg-muted/35 p-4">

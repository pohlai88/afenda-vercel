...
/**
 * Knowledge surface error boundary — keeps the dashboard shell mounted while
 * the knowledge substrate (pgvector, Vercel Blob, embeddings) recovers from
 * uncaught render or upstream service failures.
 */
export default function KnowledgeError(props: NextAppErrorPageProps) {
  const { error } = props
  ...
  const segment = envelope?.orgSlug
    ? `dashboard/${envelope.orgSlug}/knowledge`
    : "dashboard/knowledge"
  ...
  return (
    <RouteErrorShell>
      <h1 className="text-2xl font-semibold text-foreground">
        Knowledge could not load
      </h1>
      <p className="text-sm text-muted-foreground">
        Something went wrong while rendering the knowledge surface. This may be
        a transient storage or embedding issue — try again or navigate to
        another module.
      </p>

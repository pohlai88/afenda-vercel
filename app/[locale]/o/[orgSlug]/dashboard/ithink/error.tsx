...
/**
 * iThink surface error boundary — keeps the dashboard shell mounted while
 * the iThink consequence list / detail recovers from uncaught render failures.
 */
export default function IthinkError(props: NextAppErrorPageProps) {
  const { error } = props
  ...
  const segment = envelope?.orgSlug
    ? `dashboard/${envelope.orgSlug}/ithink`
    : "dashboard/ithink"
  ...
  return (
    <RouteErrorShell>
      <h1 className="text-2xl font-semibold text-foreground">
        iThink could not load
      </h1>
      <p className="text-sm text-muted-foreground">
        Something went wrong while rendering the iThink surface. The dashboard
        shell remains available — try again or navigate to another module.
      </p>

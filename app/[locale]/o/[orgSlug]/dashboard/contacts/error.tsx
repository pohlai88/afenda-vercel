...
/**
 * Contacts surface error boundary — keeps the dashboard shell mounted while
 * the contacts list / detail recovers from uncaught render failures.
 */
export default function ContactsError(props: NextAppErrorPageProps) {
  const { error } = props
  ...
  const segment = envelope?.orgSlug
    ? `dashboard/${envelope.orgSlug}/contacts`
    : "dashboard/contacts"
  ...
  return (
    <RouteErrorShell>
      ...
      <p className="text-sm text-muted-foreground">
        Something went wrong while rendering the contacts surface. The dashboard

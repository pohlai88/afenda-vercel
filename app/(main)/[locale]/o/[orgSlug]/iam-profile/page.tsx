...
import { getProfileShellData } from "#features/iam-profile/server"
...
export default async function OrganizationIamProfileIndexPage({
  params,
}: {
  ...
  const identityHref = organizationIamProfilePath(
    orgSlug,
    "identity"
  ) as Route
  const securityHref = organizationIamProfilePath(
    orgSlug,
    "security"
  ) as Route
  ...
  const membershipRows: IamProfileMembershipOrgRow[] = shellData.organizations.map(
    (org) => ({
      id: org.id,
      name: org.name,
      slug: org.slug,

...
test.describe("individual personal onething (optional credentials)", () => {
  ...
  test.beforeEach(({}, testInfo) => {
    if (!email || !password) {
      testInfo.skip(
        true,
        "Set E2E_INDIVIDUAL_EMAIL and E2E_INDIVIDUAL_PASSWORD for a verified account (e.g. after sign-up + email verification, or a seeded dev user used only for personal onething)."
      )
    }
  })

  test(
    "signed-in user opens personal onething, adds a task, marks complete",
    { tag: "@smoke" },
    async ({ page }) => {

 ...
 * exercised by `todos-smoke.spec.ts`; this spec focuses on the new
 * presentation primitives.
 */
test.describe("todo operational atom UX", () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      // Seed two distinct todos so the tail has at least one entry to click.
      ...
      // Whichever todo is currently in the tail — click it and verify ?focus=
      ...
      test.skip(
        linkCount === 0,
        "Both seeded todos collapsed onto the canvas — cannot exercise tail click in this run."
      )

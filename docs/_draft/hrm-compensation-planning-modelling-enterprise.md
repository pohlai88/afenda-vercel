...
Enterprise requirement pack (**HRM-CPM-001**–**HRM-CPM-030**) for Afenda HRM.

Implementation should follow other payroll siblings: [`lib/features/hrm/payroll-compensation/`](../../lib/features/hrm/payroll-compensation/) with a `*.contract.ts`, plus `actions/`, `components/`, `data/`, and `schemas/` as needed — not raw requirements Markdown inside `lib/features/`.

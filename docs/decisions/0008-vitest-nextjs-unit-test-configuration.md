...
| 2026-05-20 | **Vitest projects:** `unit-node` (default Node) + `unit-dom` (`*.dom.test.{ts,tsx}` + `jsdom`). RTL specs renamed to `*.dom.test.tsx`; use `pnpm test:fast:node` / `:dom`. WSL coverage: `docs/testing/vitest-wsl-coverage.md`. Import audit: `pnpm test:analyze:imports:report` → `.artifacts/reports/vitest-import-durations.txt`. |
...
| 2026-05-20 | **Artifact layout:** ad-hoc outputs under `.artifacts/reports/` and `.artifacts/logs/`; blob dir `.artifacts/vitest-reports/` via `.vitest-reports` junction — run **`pnpm artifacts:init`** after clone/pull. |

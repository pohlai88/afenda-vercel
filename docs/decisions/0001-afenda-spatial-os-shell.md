...
| **Implements in code** | **`components/workbench/`** (Workbench shell — L1/L3/L4 wiring), **`components/nexus/`** (Nexus page at `/o/{slug}/nexus` + Lynx summon), dashboard **route** layouts under `app/[locale]/o/[orgSlug]/apps/**`, nav registries adjacent to **`lib/dashboard-module-paths.ts`**, `workbench-command.tsx`, and (target) **`lib/features/command/`** for the L3 registry + intent pipeline                         |
